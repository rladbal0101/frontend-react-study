...
function ReservationRefactoring(props) {
  ...
  const handleInputChange = (e) => {
    ...
    // (inputs가 가리키는 객체의 내부 데이터만 바뀐것이지 참조값(객체의 주소값)은 변하지 않았기 때문)
    // inputs[name] = inputValue;
    // setInputs(inputs); // set함수를 써도 재렌더링 발생 안함(리액트가 객체가 바뀐것을 감지하지 못함)
    ...
    setInputs(prevInputs => ({ // 이전 state를 받아와 새로운 객체를 만들어주기
