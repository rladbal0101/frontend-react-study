...
function ApiRequest(props) {
  ...
  // 연습해보기
  const handleRequestByIdAsyncTest = async id => {
    try {
  ...
  return (
    <>
      <div>
        {/* <button type='button' onClick={() => handleRequestById}>불러오기</button> */}
        <button type='button' onClick={() => handleRequestByIdAsync(5)}>불러오기</button>
        <button type='button' onClick={() => handleRequestByIdAsyncTest(3)}>불러오기</button>
      </div>
      {data &&
        <>
